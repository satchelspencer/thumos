	...
	var api = {
		...
		verify : function(req, res, next){
			function check(r, callback){
				if(r.signedCookies.auth === undefined) callback('no cookie', false);
				else{
					var data = JSON.parse(r.signedCookies.auth);
					if(data.exptime < (new Date).getTime()) callback('expired', false); //browser sent expired cookie
					...
					if(data.hmac == hmac){
						...
						cache.get(data.uid, function(memerr, d){
							if(memerr) callback(memcached);
							else if(d) callback(null, data.uid);
							else callback('expired/not in memcached', false);
						});
						
					}else callback('hmac invalid', false); //forged/invalid cookie
				}
			}
