...
define(function(set){
	...
	function series(middlewares, input, callback, context, plural){
		...
		async.reduce(middlewares, input, function(values, middlewareDef, middlewareDone){
			...
			if(!_.isFunction(middlewareDef)){
				/* if as set is passed as middleware */
				if(middlewareDef.get) middleware = function(inp, cb){
					middlewareDef.get(inp, function(e){
	...
	return function(gname, gmodels, callback, context){
		var names = gname.split(' ');
		async.reduce(names, gmodels, function(models, name, done){
