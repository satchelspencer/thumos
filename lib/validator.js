	...
	return function(set){
		...
		return function(models, cb){
			...
			async.mapSeries(models, function(data, callback){
				...
				var dbOnly = ['_id']; //properties to ignore in model
				for(var prop in data) if(!props[prop] && dbOnly.indexOf(prop) == -1) unexpected.push(prop);
				for(var prop in props) if(!data[prop]) missing.push(prop);
