...
define(function(set, typedProps){
	...
	function validateProp(propName, dataValue, context, callback){
	...
	return function(models, cb, uid, update){ //update bool if to allow database-only properties, as well as not include all props
		var context = {
			uid : uid,
			update : update
		}
		...
		async.mapSeries(models, function(data, callback){
			...
			var priv = privateProps;
			if(uid === undefined) priv = [];
			var modelProps = _.difference(_.keys(props).concat(dbOnly), priv.concat(readOnlyProps));
			...
			/* now iterate over all the data props that are modelProps and ignore the db props */
				...
				if(!(_.has(output, propName) || _.has(data, propName))) cbProp(null, output);
					...
					validateProp(propName, dataValue, context, function(e, value){
						if(e){
							var message = e;
							e = {};
							e[propName] = message;
				...
				if(e) callback(e);
					...
					props[propName].compute(model, function(e, computedValue){
						console.log(propName, computedValue);
						if(e || computedValue === undefined) computeDone(e);
						else validateProp(propName, computedValue, context, function(e, value){
