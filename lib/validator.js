			...
			async.mapSeries(models, function(data, callback){
				...
				/* now iterate over all the data props that are modelProps and ignore the bin props */
					...
					if(props[propName].type){
						...
						if(!server) props[propName].type.encode(data[propName], function(e, en){
							if(e) cbProp(e);
							else next(en);
						});
						else server.activeTypes[props[propName].type.name].finalize(data[propName], cbProp);
