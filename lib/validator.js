			...
			async.mapSeries(models, function(data, callback){
				...
				if(_.keys(error).length){
					...
					if(props[propName].type){
						/* we gotta encode a custom type */
						var type = props[propName].type;
						/* not encoded, go for it */
						if(!type.isEncoded(data[propName])) type.encode(data[propName], set, propName, function(e, encoded){
							if(e) cbProp(e);
							else{
								data[propName] = encoded;
								validate();
							}
						});
						else if(!update) cbProp('already encoded'); //must not be encoded if adding
						else cbProp(); //if updating
					}else validate();
					function validate(){
