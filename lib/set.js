define(['underscore', './validator', 'async'], function(_, validator, async){
	return function(nodeRequire){
		return function(config){
			return function(thumos){
				var sconfig = thumos.config;
				...
				var api = {
					...
					fn : {},
					config : config,
					/* bring in from thumos */
					db : {
						collection : thumos.db.collection(config.collection||config.name),
						id : thumos.db.id,
						str : thumos.db.str
					}
				}
