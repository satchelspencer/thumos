    						...
    						api.valid(data, function(e, models){ //make sure its valid
    							if(e) callback(e);
    								...
    								if(!model._id) cb({index : index, missing : '_id'}); //gotta have an id to update
    									...
    									collection.findOne({_id : model._id}, selObj, function(e, oldModel){ //get its current value
    										if(e) cb(e);
    											...
    											async.each(_.keys(toPurge), function(prop, purged){
    												...
    												if(e) cb(e);
    												else collection.update({_id : model._id}, {$set : model}, function(e, result){
